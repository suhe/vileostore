...
<div class="side-menu animate-dropdown outer-bottom-xs">
    <div class="head"><i class="icon fa fa-align-justify fa-fw"></i><?=Yii::t('app','category')?></div>        
    <nav class="yamm megamenu-horizontal" role="navigation">	
        <ul class="nav">
	    <?php foreach(\common\models\Category::getNestedCategory(1,0) as $nav){ ?>
            <li class="dropdown menu-item">
                <a href="<?=\yii\helpers\Url::to(['product/category/','id'=>$nav->id,'slug'=>$nav->slug])?>" class="dropdown-toggle" data-toggle="dropdown"><i class="<?=isset($nav->icon)?$nav->icon:'icon fa fa-external-link fa-fw'?>"></i><?=$nav->name?></a>
                <!-- ================================== MEGAMENU VERTICAL ================================== -->
		...
		<ul class="dropdown-menu mega-menu">
			...
			<div class="row" style="min-height:370px">
				...
				<ul>
				    ...
				    <li><a href="<?=\yii\helpers\Url::to(['product/category/','id'=>$sub_nav->id,'slug'=>$sub_nav->slug])?>"> <i class="<?=isset($nav->icon)?$nav->icon:'icon fa fa-external-link fa-fw'?>"></i> <?=$sub_nav->name?></a></li>
				    <?php } ?>
				</ul>
			    </div>
